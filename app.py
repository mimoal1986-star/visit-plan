...
# Шаблоны - отдельный раздел
# Раздел загрузки файла
st.header("📤 Загрузка файла")

tab1, tab2, tab3 = st.tabs(["📁 Загрузка файла", "📥 Скачать шаблон", "📋 Описание полей"])

with tab1:
    st.subheader("Загрузите файл с данными")
    uploaded_file = st.file_uploader("Выберите файл Excel", type=['xlsx', 'xls'], label_visibility="collapsed")
    
    if uploaded_file is not None:
        st.success(f"✅ Файл '{uploaded_file.name}' загружен успешно!")
        st.info("Теперь нажмите кнопку 'Рассчитать план' для обработки данных")
    else:
        st.info("👆 Загрузите файл Excel с данными для расчета плана")
    
    st.markdown("""
    **Требования к файлу:**
    - Формат: Excel (.xlsx или .xls)
    - Должен содержать 2 вкладки
    - Кодировка: UTF-8
    - Максимальный размер: 10 МБ
    """)

with tab2:
    st.subheader("Шаблон файла данных")
    template_data = create_template()
    st.markdown(get_download_link(template_data, "шаблон_данных.xlsx", "📥 Скачать шаблон Excel"), unsafe_allow_html=True)
    
    st.markdown("""
    **Файл содержит 2 вкладки:**
    ...
    **Как использовать:**
    ...
    2. Заполните данные
    3. Сохраните файл
    4. Загрузите в сервис для расчета
    """)
    
    # Быстрая загрузка тестовых данных
    st.markdown("---")
    st.subheader("Быстрый старт")
    st.info("Для тестирования можно использовать шаблон с тестовыми данными")

with tab3:
    st.subheader("Описание полей в шаблоне")
    ...
    with col1:
        ...
        **Обязательные поля:**
        ...
        - `Тип` - тип точки (Мини/Гипер/Супер)
        
        **Необязательные поля:**
        - `Адрес` - можно определить по координатам
        - `Город` - можно определить по координатам
        - `Кол-во_посещений` - план посещений (по умолчанию 1)
        """)
    
    with col2:
        st.markdown("""
        ### Вкладка 'Факт_посещений' (факт)
        
        **Обязательные поля:**
        - `ID_Точки` - идентификатор точки (должен соответствовать ID из вкладки Точки)
        - `Дата_визита` - дата фактического посещения
        
        **Формат данных:**
        - Одна строка = один визит
        - Если точка посещалась 3 раза - должно быть 3 строки
        - Дата в формате ДД.ММ.ГГГГ или ГГГГ-ММ-ДД
        - Можно оставить пустым, если фактические данные отсутствуют
        """)
    ...
    try:
        ...
        if not city_stats_df.empty:
            ...
            with pd.ExcelWriter(excel_buffer, engine='openpyxl') as writer:
                city_stats_display.to_excel(writer, sheet_name='Статистика по городам', index=False)
...
if 'summary_df' in st.session_state and not st.session_state['summary_df'].empty:
    ...
    with results_tabs[1]:
        ...
        with col1:
            ...
            selected_city = st.selectbox("Город", cities, key="city_filter_summary")
        
        with col2:
            auditors = ['Все'] + sorted(summary_df['Аудитор'].dropna().unique().tolist())
            selected_auditor = st.selectbox("Аудитор", auditors, key="auditor_filter_summary")
        ...
        if not filtered_df.empty:
            ...
            display_cols = ['Аудитор', 'Город', 'ISO_Неделя', 'Начало_недели', 
                          'Конец_недели', 'План_посещений']
            
            # Если есть факт, добавляем эти колонки
            ...
            display_df = display_df.rename(columns={
                'ISO_Неделя': 'Неделя',
                'Начало_недели': 'Начало недели',
                'Конец_недели': 'Конец недели',
                'План_посещений': 'План',
                'Факт_посещений': 'Факт',
                '%_выполнения': '% выполнения'
            })
            ...
            # Сортируем
            display_df = display_df.sort_values(['Город', 'Аудитор', 'Неделя'])
...
st.caption(
    ...
    "**Примечания:**  \n"
    "1. Для тестирования используйте шаблон с тестовыми данными  \n"
    "2. После расчета перейдите во вкладки для просмотра результатов  \n"
    "3. Полигоны генерируются автоматически при нажатии кнопки в боковой панели"
)
