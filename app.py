...
if st.button("🚀 Рассчитать план", type="primary", use_container_width=True):
    ...
    if 'data_file' not in locals() and 'data_file' not in st.session_state:
        st.error("⚠️ Пожалуйста, загрузите файл с данными!")
        st.stop()
    
    # Получаем файл из session state или локальной переменной
    data_file = st.session_state.get('data_file', locals().get('data_file'))
